	...
	tests := []struct {
	...
	}{
		{
			...
			messages: []openai.Message{
				{
					Role:      "user",
					Content:   openai.MessageContent{Content: new("Hello")},
					Reasoning: new("thinking about the question"),
				},
			},
			...
			messages: []openai.Message{
				{
					Role:             "assistant",
					Content:          openai.MessageContent{Content: new("Hello")},
					ReasoningContent: new("reasoning content here"),
				},
			},
			validate: func(t *testing.T, messages []openai.Message) {
			...
			messages: []openai.Message{
				{
					...
					ReasoningContent: new("reasoning content"),
				},
			},
			validate: func(t *testing.T, messages []openai.Message) {
				assert.Nil(t, messages[0].Reasoning, "Reasoning should be nil")
		...
		},
		{
			...
			messages: []openai.Message{
				{
					Role:    "user",
					Content: openai.MessageContent{Content: new("Hello world")},
				},
			},
			validate: func(t *testing.T, messages []openai.Message) {
	...
	httpReq, err := transformer.TransformRequest(context.Background(), &llm.Request{
		...
		Messages: []llm.Message{
			...
			},
			{
				...
				Reasoning:        new("assistant reasoning"),
				ReasoningContent: new("reasoning content"),
			},
		},
	})
